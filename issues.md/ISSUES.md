...
- [x] [PG-107] Let any authenticated Pinguin user add a sender domain, receive exact DNS setup records, manually check DNS, and create/use SMTP relay identities only after that domain verifies. Resolved with owner-scoped sender domains, generated TXT/SPF/DMARC setup instructions, a manual DNS check API/UI action, verified-domain gates for identity creation plus SMTP auth/forwarding, dynamic public sender domains without TAuth changes, docs/config updates, and passing `make ci`.
- [x] [PG-365] Allow admins to open an existing SMTP relay identity password in the Gmail SMTP settings modal and rotate the credentials from inside that modal. Resolved with encrypted-at-rest SMTP identity passwords, a protected credentials retrieval endpoint, a View password modal flow, in-modal rotation, docs updates, and passing `make test`, `make lint`, and `make ci`.
...
- [x] [PG-362] Align Pinguin SMTP setup with the gateway high-port publishing contract. Resolved by switching local SMTP Compose ports to `8025:25` and `8465:465`, normalizing shared-address MX docs/config examples to `mx.pinguin.mprlab.com`, and adding a deploy preflight that rejects mprlab-gateway checkouts missing the `8025/8465` Caddy host-port contract before running `deploy TARGET=pinguin`.
...
- [x] [PG-389] The repository still split deployment policy across a schema-v3 `resources.yml` and `.mprlab/release.yml`, while its black-box contract continued to require the removed image-visibility field. Resolved by making schema-v4 `resources.yml` the sole deployment manifest, embedding `release.scheme: semver`, deleting the obsolete release file, and updating the deployment contract to reject visibility and prove the current shape; the complete `make ci` gate passed with 100% Go statement coverage and all 51 Playwright scenarios.
- [x] [PG-388] `make release` could fail intermittently in `test-frontend` because the browser fixture raced its own `MPRUI`/user-menu mutations against late shared-shell orchestration, while the landing snapshot captured the still-hydrating session surface. Resolved by representing restored authentication with the real test cookie plus `mpr-ui` restore hint, making `/auth/session` the sole profile authority, carrying runtime fixture state through one deterministic cookie contract, waiting for `Workspace ready` before the landing snapshot, and refreshing that settled baseline. The profile restoration and visual contracts each passed 20 consecutive repetitions, and all 51 scenarios passed through `make test-frontend`.
...
- [x] [PG-366] `pinguin-doctor` can still report tenant configs with legacy or unknown nested tenant fields as valid because the custom tenant YAML decoder bypasses strict known-field checks inside `yaml.Node.Decode`. Resolved by making tenant bootstrap YAML decoding strict for current fields only, rejecting legacy `status` and `identity` shapes at the config edge, making doctor reuse the runtime tenant schema, validating referenced `tenants.configPath` files, and passing `make test`, `make lint`, and `make ci`.
- [x] [PG-364] Preserve the shared-address relay workflow for SES-backed inbound fanout: owners send as a registered shared address, replies to that address fan out to all forwarding owners, and owner replies naturally go back to the original external sender while using the shared address as the outgoing identity. Resolved by rewriting owner fanout copies to keep the shared address as the SES-verified `From` mailbox, preserve the original sender display label, preserve reply routing via `Reply-To`, strip stale authentication headers, keep trace and resent headers in original transport order, pass through the accepted raw message when the rewrite parser cannot safely transform it, and add regression coverage.
- [x] [PG-363] Gateway deploy preflight for Pinguin started the normal server instead of `pinguin-doctor` because the production image only shipped `/usr/local/bin/pinguin` as an `ENTRYPOINT`. Resolved by publishing the doctor binary in the image, making the server the default `CMD` so Compose run commands can override it, adding a Dockerfile contract test, validating the exact gateway doctor command against current config, and passing `make ci`.
- [x] [PG-358] `pinguin-doctor --expand-env` rejected current production-shaped configs with `server.tauth` because the doctor still validated legacy `web.tauth.signingKey` and `web.tauth.issuer` fields. Resolved by aligning doctor TAuth parsing and validation with the runtime `server.tauth` schema, adding current-shape regression coverage, and validating the gateway Pinguin config with env expansion.
...
- [x] [PG-366] Production `pinguin-api` can crash during startup after SMTP sender-domain ownership support because legacy configured rows migrated with `owner_email` as NULL are not deleted before the configured allowlist is re-seeded, causing `UNIQUE constraint failed: sender_domains.domain` and gateway `/runtime-config` 502s while downstream services such as LoopAware time out connecting to Pinguin. Resolved by deleting legacy NULL-owner configured sender-domain rows during allowlist replacement without compatibility migration support, adding regression coverage, and passing `make test`, `make lint`, and `make ci`.
- [x] [PG-368] `make deploy` could complete without first proving the operator was on clean local `master` matching `origin/master` with no open PRs, leaving branch state ambiguous for production operations. Resolved by adding a shared production Git guard to `make release`, `make publish`, and `make deploy`; the guard prints the verified branch/commit, rejects non-master or dirty worktrees, requires local `master` to equal `origin/master`, requires zero open PRs, updates docs, adds contract coverage, and passes focused tests.
- [x] [PG-367] SMTP relay identity rows exposed password and rotate controls directly in the table. Resolved by making the sender address open the Gmail SMTP settings modal, removing `View password` and `Rotate` from table actions, rendering the password as plain modal text with copy support, keeping rotation inside the modal, and adding Playwright coverage plus passing `make test-frontend`, `make test`, and `make ci`.
- [x] [PG-369] `make release` failed in `tests/config` because production workflow contract tests still expected the older release environment default syntax and generic gateway deploy target. Resolved by aligning the contract tests and README with the current explicit release defaults and `deploy-pinguin-backend` target, with focused config coverage passing.
- [x] [PG-370] `make deploy` still treated GitHub Pages as a Pinguin-local post-Ansible step, so the gateway Ansible run could complete without owning the static frontend publication. Resolved by exposing a `pages-deploy` target for legacy branch publishing, moving deploy orchestration to gateway `deploy-pinguin` so Ansible owns the Pages resource, keeping `--skip-pages` on the backend-only target, and updating the deploy contract tests/docs.
- [x] [PG-371] SMTP relay rows created before retrievable password storage remained visible even though their password could not be reopened. Resolved by running a startup data migration that rotates active rows with missing password ciphertext into current credential data before the SMTP identity repository starts; normal credential reads stay on the current data path.
- [x] [PG-372] SMTP sender-domain onboarding still had a split-brain contract: production config seeded operator-owned verified sender domains while the SMTP relay page was supposed to let authenticated users add arbitrary domains through DNS verification. Resolved by making sender domains dynamic DB/API records only, deleting legacy unowned configured-domain rows at startup, rejecting `smtpSubmission.senderDomains` in config/doctor, removing static sender-domain template/env references, and surfacing stable sender-domain API errors in the UI. Validated with focused Go/browser regressions, `make test`, `make lint`, and `make ci`.
...
- [x] [PG-380] `make release` failed at `test-frontend` because the live `mpr-ui@latest` loader rejected Pinguin's retired `authButton` YAML before upgrading the shared shell. Resolved by moving every browser environment to the current `/auth/session` restoration contract, deleting the obsolete presentation config, aligning the Playwright TAuth fixture and anonymous-startup assertions, documenting the boundary, and passing `make test`, `make lint`, `make ci`, and all 35 browser scenarios.
- [x] [PG-381] `make deploy` rejected the current mprlab-gateway checkout before dispatch because Pinguin still required retired product-specific SMTP verification keys instead of treating `deploy-pinguin-backend` as the canonical gateway-owned boundary. Resolved by deleting the duplicate private-file preflight, delegating gateway preflight/deploy/verification to the public target, adding a black-box command-boundary regression, and passing `make test`, `make lint`, `make ci`, 100.0% Go statement coverage, and all 35 browser scenarios.
