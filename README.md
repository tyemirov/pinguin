...
- **Authenticated SMTP Submission:**
  Optionally accepts Gmail-compatible SMTP AUTH submissions for exact sender identities and relays the raw message through the tenant’s configured upstream SMTP provider.
- **Email Attachments:**  
...
- **SMTP Send-As Identities:**
  Dashboard users can create, rotate, and delete one-address SMTP credentials for Gmail Send-As. Generated passwords are shown once and are never stored in retrievable form.
...
- `tenants[].senderDomains` (list of strings, optional): domains that may be used for dashboard-created SMTP submission identities.
  - This allowlist is intentionally separate from HTTP routing domains.
  - If omitted, the tenant cannot create SMTP Send-As identities until a sender domain is configured.
...
Pinguin can optionally expose a Gmail-compatible SMTP submission endpoint. This is for outbound “send as” use cases only; inbound forwarding and mailbox hosting remain outside Pinguin.
...
smtpSubmission:
  ...
  listenAddr: :587
  tlsListenAddr: :465
  tlsCertPath: /certs/smtp.example.com.crt
  tlsKeyPath: /certs/smtp.example.com.key
  maxMessageBytes: 26214400
  maxRecipients: 100
  allowInsecureAuth: false
```

Then configure each tenant’s outbound sender domains:

```yaml
tenants:
  - id: tenant-acme
    domains: [app.acme.example]
    senderDomains: [acme.example]
```

Dashboard workflow:

1. Sign in to the Pinguin dashboard.
2. Open **SMTP identities**.
...
Pinguin validates that the SMTP login, envelope sender, and RFC 5322 `From` header all match the exact identity. Accepted messages are relayed through the tenant’s configured `emailProfile`; direct-to-recipient-MX delivery, mailbox hosting, bounce processing, DKIM signing, and domain reputation management remain with the upstream SMTP provider.
