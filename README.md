...
`smtpSubmission.senderDomains` is the global allowlist for exact sender identities such as `alice@example.com`. `smtpSubmission.relay` is the upstream SMTP account Pinguin uses after accepting a message; it is intentionally separate from notification tenants and `tenants[].emailProfile`.
