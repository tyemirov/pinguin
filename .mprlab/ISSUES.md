...
Entries record newly discovered requests or changes. Resolved historical entries live in `issues.md/ISSUES.md`.
...
- [ ] [P001] Keep public SMTPS on port 465 while routing Caddy to Pinguin's canonical private plaintext SMTP submission listener on port 587.
  Goal:
  Preserve the public SMTPS port-465 contract while proving the deployed TLS dialogue terminates at Caddy and reaches Pinguin's canonical private plaintext SMTP submission listener on port 587.
  Requirements:
  - Keep the public SMTPS experience on port 465.
  - Route Caddy to the canonical private plaintext SMTP submission listener on port 587.
  - Prove the public TLS SMTP dialogue with real mail software on the disposable deployment host before promotion.
  Deliverables:
  - A concrete implementation and validation plan for the 465-to-587 routing contract.
  - Evidence from real mail software on the disposable deployment host.
  Validation:
  - Prove the public TLS SMTP dialogue with real mail software on the disposable deployment host.
  Context:
  - Migrated from legacy planning entry `PG-386` during the 2026-08-09 backlog cleanup.
