...
- `smtpSubmission` controls optional SMTP submission listeners, optional operator-seeded verified sender domains, user-owned sender-domain DNS verification, public Gmail-facing SMTP settings, and the selected delivery mode. Production runs behind gateway-owned Caddy Layer 4 SMTPS termination by advertising edge `465` / `ssl` while the edge gateway forwards to `tutosh:8465`, Docker publishes that host port to Caddy `:465`, and Pinguin listens privately on plaintext SMTP inside the Docker network.
