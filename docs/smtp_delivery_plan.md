...
- **smtpSubmission.publicPort** and **smtpSubmission.publicSecurityMode** control the Gmail-facing settings shown with one-time SMTP identity credentials. These differ from the private listener in production: the schema-v3 gateway exposes `pinguin-api.mprlab.com:465`, terminates TLS in Caddy, and forwards plaintext SMTP to Pinguin's private submission capability.
...
The schema-v3 gateway accepts public SMTP on `mx.pinguin.mprlab.com:25` and proxies it through Caddy to Pinguin's private forwarding capability.
